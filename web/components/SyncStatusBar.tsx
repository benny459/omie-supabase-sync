...
export default function SyncStatusBar() {
  ...
  return (
    <div className="relative inline-block">
      <button
        onClick={() => setOpen(o => !o)}
        className="inline-flex items-center gap-2 px-3 py-1 text-[11px] text-ww-textMuted hover:text-ww-text hover:bg-ww-bg rounded-md transition"
        title="Histórico de sincronização"
      >
        <span className={`w-1.5 h-1.5 rounded-full ${hasErr ? "bg-amber-500" : "bg-emerald-500 animate-pulse"}`} />
        <span>
          Última sync: <strong className="text-ww-text">{timeAgo(latest.last_sync_at)}</strong>
          <span className="text-ww-textFaint mx-1">·</span>
          <span className="text-ww-textMuted">{humanModule(latest.modulo)}</span>
        </span>
      ...
      {open && (
        <div className="absolute left-0 top-full mt-1 z-40 w-[420px] max-h-[360px] overflow-y-auto bg-ww-panel border border-ww-border rounded-lg shadow-xl">
          <div className="px-3 py-2 border-b border-ww-border bg-ww-rowHover/70 flex items-center justify-between">
            <span className="text-[11px] font-semibold text-ww-textMuted">Últimos {syncs.length} sync(s)</span>
            <button onClick={() => setOpen(false)} className="text-ww-textFaint hover:text-ww-textMuted text-sm">×</button>
          </div>
          <table className="w-full text-[11px]">
            <thead className="sticky top-0 bg-ww-panel">
              <tr className="text-left text-[9px] uppercase tracking-wider text-ww-textMuted border-b border-ww-border">
            ...
            <tbody className="divide-y divide-slate-100">
              {syncs.map((s, i) => (
                <tr key={i} className="hover:bg-ww-rowHover">
                  <td className="px-3 py-1.5 text-ww-text font-medium">{humanModule(s.modulo)}</td>
                  <td className="px-2 py-1.5 text-ww-textMuted">{s.empresa}</td>
                  <td className="px-2 py-1.5 text-right tabular-nums text-ww-textMuted">
                    {s.total_registros != null ? s.total_registros.toLocaleString("pt-BR") : "—"}
                  </td>
                  <td className="px-2 py-1.5 text-right text-ww-textMuted" title={new Date(s.last_sync_at).toLocaleString("pt-BR")}>
                  ...
                  <td className="px-2 py-1.5 text-center">
                    {s.ultima_execucao_status === "SUCESSO" ? (
                      ...
                      <span className="inline-flex px-1.5 py-0.5 rounded-full text-[9px] font-semibold bg-ww-bg text-ww-textMuted">—</span>
                    )}
                  </td>
                </tr>
              ))}
