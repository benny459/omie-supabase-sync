    ...
    <div className="space-y-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className="relative flex-1 min-w-[220px]">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-ww-textFaint" strokeLinecap="round">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-ww-border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-500/40 focus:border-sky-500/60 bg-ww-panel"
          />
        </div>
        ...
        {hasActiveFilters && (
          <button
            onClick={clearAll}
            className="px-3 py-2 text-xs font-medium text-ww-textMuted hover:text-ww-text hover:bg-ww-bg rounded-lg transition whitespace-nowrap"
          >
            Limpar tudo
          </button>
        )}
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute z-40 mt-1 right-0 w-[260px] bg-ww-panel border border-ww-border rounded-lg shadow-xl overflow-hidden">
          <div className="p-2 border-b border-ww-border">
            <input
              autoFocus
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder={`Buscar ${label.toLowerCase()}…`}
              className="w-full px-2 py-1.5 text-xs border border-ww-border rounded-md focus:outline-none focus:ring-2 focus:ring-sky-500/40"
            />
          </div>
              ...
              return (
                ...
                >
                  <span className={`flex items-center justify-center w-4 h-4 rounded border shrink-0 ${
                    on ? "bg-sky-600 border-sky-600" : "border-ww-border bg-ww-panel"
                  }`}>
                    {on && (
                  ...
                  <span className="flex-1 text-[11px] truncate text-ww-textMuted">{val}</span>
                  <span className="text-[10px] font-semibold text-ww-textMuted tabular-nums">
                    {count}
                  </span>
                </button>
              );
          ...
          {count > 0 && (
            <div className="border-t border-ww-border p-1">
              <button
                onClick={() => { onClear(); setOpen(false); }}
                className="w-full text-center px-3 py-1.5 text-[11px] font-medium text-ww-textMuted hover:text-ww-text hover:bg-ww-rowHover rounded-md transition"
              >
...
const TONE_MAP: Record<string, { ring: string; text: string; iconBg: string }> = {
  slate:   { ring: "ring-slate-300",   text: "text-ww-text",   iconBg: "bg-ww-bg text-ww-textMuted" },
...
function SummaryCard({
  ...
  return (
    <button
      onClick={onClick}
      className={`relative bg-ww-panel rounded-xl border transition-all text-left p-4 hover:shadow-md hover:-translate-y-0.5 ${
        active
          ? `border-transparent ring-2 ${t.ring} shadow-md`
          : "border-ww-border shadow-sm"
      }`}
    >
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-ww-textMuted font-medium">{label}</div>
        </div>
      </div>
    </button>
  );
