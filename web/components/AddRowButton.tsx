      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4"
          ...
          <div onClick={(e) => e.stopPropagation()}
               className="bg-ww-panel rounded-xl shadow-2xl max-w-sm w-full p-5 space-y-4">
            ...
            {needsPvSelect && (
              <div>
                <label className="block text-[11px] font-medium text-ww-textMuted mb-1">PV/OS</label>
                <select
            ...
            <div>
              <label className="block text-[11px] font-medium text-ww-textMuted mb-1">Quantidade</label>
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-20 px-3 py-2 border border-ww-border rounded-md text-sm text-center focus:outline-none focus:ring-2 focus:ring-amber-400"
                />
                <div className="flex gap-1">
                  {[1, 3, 5, 10].map(n => (
                    <button key={n} type="button" onClick={() => setQty(n)}
                      className="px-2 py-1 text-[11px] font-medium border border-ww-border rounded-md hover:bg-ww-rowHover text-ww-textMuted">
                      {n}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex justify-end gap-2">
              <button onClick={() => setOpen(false)} className="px-3 py-1.5 text-xs font-medium text-ww-textMuted hover:bg-ww-bg rounded-md transition">Cancelar</button>
